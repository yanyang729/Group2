...
class Preprocess:
    ...
    @staticmethod
    def process_X(df, max_features=10000):
        vectorizer = TfidfVectorizer(tokenizer=tokenizer, max_features=max_features)
        X_train = vectorizer.fit_transform(pd.Series(df))
        return X_train

    @staticmethod
    def process_Y(df,max_features=10000):
        tag_result = Preprocess.tag_transforms(df.tags)
